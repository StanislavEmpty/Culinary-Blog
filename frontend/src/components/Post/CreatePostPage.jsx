...
const CreatePostPage = () => {
        ...
        const getData = async () => {
            try {
                ...
                if (getIngredientsResp.status === 200) {
                    setIngredients(getIngredientsResp.data); // Store ingredient objects
                }
            } catch (e) {
                console.log('Error.' + e);
            }
    ...
    const handleIngredientChange = async (event, newValue) => {
        ...
        setPost((prevPost) => ({ ...prevPost, ingredients: newValue })); // Update selected ingredients
    };

    const handleSubmit = async () => {
        post.stages.forEach((stage) => stage.id = 0);
        try {
            const resp = await apiService.post('/api/posts', post);
            if(resp.status === 201)
            {
                navigate(-1);
            }
